  ...
  return (
    <div className="space-y-6 pb-16 animate-fade-in">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Pengaturan Sistem & Gateway Pembayaran</h2>
        ...
        {!isAdmin && (
          <div className="px-3 py-1.5 bg-indigo-50 border border-indigo-200 text-indigo-900 rounded-lg text-xs font-bold font-mono flex items-center gap-1.5">
      ...
      <form onSubmit={handleSaveSettings} className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Nama Perusahaan / Entitas</label>
              <input
                type="text"
                disabled={!isAdmin}
                value={companyName}
                onChange={(e) => setCompanyName(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-900 focus:outline-none focus:border-indigo-600 disabled:opacity-60"
              />
            </div>

            <div>
              <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Email Kontak</label>
              <input
                type="email"
                disabled={!isAdmin}
                value={companyEmail}
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Nomor Telepon Kantor</label>
              <input
                type="text"
                disabled={!isAdmin}
                value={companyPhone}
                onChange={(e) => setCompanyPhone(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-mono text-slate-900 focus:outline-none focus:border-indigo-600 disabled:opacity-60"
              />
            </div>

            <div>
              <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Mata Uang & Zona Waktu</label>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-6">
          ...
          <div className={`p-5 rounded-xl border ${mockActive ? 'bg-white border-emerald-200 shadow-xs' : 'bg-slate-50 border-slate-200'} space-y-4 transition-colors`}>
            <div className="flex items-center justify-between">
              <div>
                <span className="font-bold text-sm text-slate-900">Simulator QRIS Mock (Uji Coba Internal)</span>
            ...
            <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Batas Kedaluwarsa QRIS (Menit)</label>
                <input
                  type="number"
                  disabled={!isAdmin}
                  value={mockExpiry}
                  onChange={(e) => setMockExpiry(e.target.value)}
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-mono font-bold text-slate-800 focus:outline-none focus:border-indigo-600"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Mode Lingkungan Server</label>
                <input
                  type="text"
                  disabled
          ...
          <div className={`p-5 rounded-xl border ${danaActive ? 'bg-white border-emerald-200 shadow-xs' : 'bg-slate-50 border-slate-200'} space-y-4 transition-colors`}>
            <div className="flex items-center justify-between">
              <div>
                <span className="font-bold text-sm text-slate-900">Integrasi DANA Bisnis (Resmi)</span>
                <p className="text-xs text-slate-500 mt-1">
            ...
            <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">DANA Client ID</label>
                <input
                  type="text"
                  disabled={!isAdmin}
                  value={danaClientId}
                  ...
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-mono text-slate-800 disabled:opacity-60 focus:outline-none focus:border-indigo-600"
                />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">DANA Client Secret (Rahasia)</label>
                <input
                  type="password"
                  disabled={!isAdmin}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Lingkungan API (API Environment)</label>
                <select
                  disabled={!isAdmin}
                  value={danaEnv}
                  onChange={(e) => setDanaEnv(e.target.value)}
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-800 focus:outline-none focus:border-indigo-600"
                >
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Alamat Webhook (Otomatis)</label>
                <input
                  type="text"
                  disabled
          ...
          <div className={`p-5 rounded-xl border ${internalMerchantActive ? 'bg-white border-emerald-200 shadow-xs' : 'bg-slate-50 border-slate-200'} space-y-4 transition-colors`}>
            <div className="flex items-center justify-between">
              <div>
                <span className="font-bold text-sm text-slate-900">QRIS Statis Kantor (Manual)</span>
                <p className="text-xs text-slate-500 mt-1">
            ...
            <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Nama Toko (Merchant)</label>
                <input
                  type="text"
                  disabled={!isAdmin}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Nomor NMID (National Merchant ID)</label>
                <input
                  type="text"
                  disabled={!isAdmin}
              ...
              <div className="md:col-span-2">
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">Gambar QRIS (Max 1MB, format PNG/JPG)</label>
                
                <div className="flex items-start gap-4">
                  {(internalMerchantImagePath || internalMerchantImageBase64) ? (
                    <div className="relative border border-slate-200 rounded-lg bg-white p-2">
                      <img 
                    ...
                    <div className="w-28 h-28 flex items-center justify-center border-2 border-dashed border-slate-300 rounded-lg bg-slate-50 text-slate-400 text-xs text-center p-2">
                      Belum ada QRIS
                    </div>
                  )}
                  ...
                  <div className="flex-1 mt-2">
                    ...
                    />
                    <p className="text-[11px] text-slate-500 mt-2">File akan langsung diubah bentuk formatnya sebelum disimpan ke server.</p>
                  </div>
                </div>
              </div>
            </div>
